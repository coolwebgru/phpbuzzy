...
$logincontent = '<div class="loginbox">
	<div class="typerq">
		<a href="javascript:;" id="chooseiType"><i class="fa fa-user-secret"></i><span class="name">Guest</span> <span class="fa fa-angle-down"></span></a>
		<ul class="dropdown-menu">
		<li><a href="javascript:;" onclick="chooseType(\'Register\',1, this);" class="selected"><i class="fa fa-user-secret"></i>Guest <i class="fa fa-selected fa-check"></i></a></li>
           <li><a href="javascript:;" onclick="chooseType(\'Login\',0, this);"><i class="fa fa-sign-in"></i>Login <i class="fa fa-selected fa-check"></i></a></li>
...
$satir_sayisi  = $dbpdo->query("SELECT count(*) from yorumlar where icerikid = '".$yorumicerikid."' and domainaccess = '".$domainaccess."' and onay = '1'")->fetchColumn();
$sayfa          = $sayfaid; 
...
if($yorumdesc=="populer"){
	...
	if($yorumdesc=="populer"){
		...
		FROM yorumlar A where A.icerikid='$yorumicerikid' and A.onay = '1' and  A.domainaccess = '$domainaccess'
		) T where T.memberCount >= '10'
	 ORDER BY T.memberCount DESC
    limit 0, $db_populercommentcount");
	}ELSEif($yorumdesc=="user"){
		$stmtooo  = $dbpdo->query("SELECT * FROM yorumlar where ekleyen='$yorumicerikid' and onay = '1' and domainaccess = '$domainaccess' ORDER BY tarih DESC limit $goster, $limit");
	}ELSE{
		if($yorumicerikdesc=="best"){
			...
			$stmtooo  = $dbpdo->query("SELECT * FROM yorumlar  where İcerikid='$yorumicerikid' and onay = '1' and domainaccess = '$domainaccess' ORDER BY tarih ASC limit $goster, $limit");
		}ELSE{
			$stmtooo  = $dbpdo->query("SELECT * FROM yorumlar  where İcerikid='$yorumicerikid' and onay = '1' and domainaccess = '$domainaccess' ORDER BY tarih DESC limit $goster, $limit");
		}
...
foreach ($rowingele as $gelenl){	
				...
				if($out_id > 0){
					$yorumyazanid="";
					$yorumyazankulladi=$out_name;
					$yorumyazankulladi2='href="'.$out_link.'" target=_blank';
					$yorumyazanicon=$out_icon;
					$yorumyazanunvan="1";

				}elseif($ekleyen==0){
					$yorumyazanid="";
					$yorumyazankulladi=$u_name;
					$yorumyazankulladi2='href="javascript:;"';
					...
					$yorumyazankulladi2='href="javascript:;" onclick="profilego('.$yorumyazanid.')"';
					$yorumyazanicon=resimcreate($gelenm["icon"],"s","member/avatar");
					$yorumyazanunvan=$gelenm["unvan"];
				}

				if($out_id > 0){
					$yorumyazanunvancolor="";$yorumyazanunvanyazi="";
				}elseif($ekleyen==0){
 ...
 <a '.$yorumyazanunvancolor.' '.$yorumyazankulladi2.' data-id="'.$yorumyazanid.'" data-user="'.$yorumyazankulladi.'" onmouseover="loaduserWidget(this)">'.$yorumyazankulladi.''.$yorumyazanunvanyazi.'</a>
...
$veri="";
		...
		$rsqalcev  = $dbpdo->query("select * from yorumlar where icerikid = '".$yorumid."' and tip = 'yorumcevap' and onay = '1' order by tarih asc limit ".$goster.", ".$limit."");
		...
		$u_email	=  $gelenlcev['u_email'];
			...
			if($out_id > 0){
				$cevapyazanid="";
				$cevapyazankulladi=$out_name;
				$cevapyazankulladi2='href="'.$out_link.'" target=_blank';
				$cevapyazanicon=$out_icon;
				$cevapyazanunvan="1";

			}elseif($cevapekleyen==0){
					$cevapyazanid="";
					$cevapyazankulladi=$u_name;
					$cevapyazankulladi2='href="javascript:;"';
					...
					$cevapyazankulladi2=' href="javascript:;" onclick="profilego('.$cevapyazanid.')"';
				...
				if($out_id > 0){
				$cevapyazanunvancolor="";$cevapyazanunvanyazi="";
				}elseif($cevapekleyen==0){
...
<div class="c-text"><div class="c-top"> <span class="report">'.$islemler.'</span><a '.$cevapyazanunvancolor.'  '.$cevapyazankulladi2.' data-id="'.$cevapyazanid.'"  data-user="'.$cevapyazankulladi.'" onmouseover="loaduserWidget(this)">'.$cevapyazankulladi.''.$cevapyazanunvanyazi.'</a>
...
$veri="";
		...
		if($yorummesajsay > "0"){
			...
			$rsqasd  = $dbpdo->query("select * from yorumlar where icerikid = '".$cevapid."' and tip = 'yorumcevapyanit' and onay = '1' order by tarih asc limit $goster, $limit");
		...
		$u_email	=  $gelaslcev['u_email'];
			...
			if($out_id > 0){
				$cevapyaniyazanid="";
				$cevapyaniyazankulladi=$out_name;
				$cevapyaniyazankulladi2='href="'.$out_link.'" target=_blank';
				$cevapyaniyazanicon=$out_icon;
				$cevapyaniyazanunvan="1";

			}elseif($cevapyaniekleyen==0){

				$cevapyaniyazanid="";
				$cevapyaniyazankulladi=$u_name;
				$cevapyaniyazankulladi2=' href="javascript:;" ';
				...
				$cevapyaniyazankulladi2=' href="javascript:;" onclick="profilego('.$cevapyaniyazanid.')"';
...
<div class="c-text"><div class="c-top"><span class="report">'.$islemler.'</span><a '.$cevapyaniyazanunvancolor.' '.$cevapyaniyazankulladi2.' data-id="'.$cevapyaniyazanid.'" data-user="'.$cevapyaniyazankulladi.'" onmouseover="loaduserWidget(this)">'.$cevapyaniyazankulladi.''.$cevapyaniyazanunvanyazi.'</a>
