    ...
    return $.ajax({
     ...
     success: function(t) {
      return t.error ? (alert(t.message), !1) : ("prepend" == a ? $("#" + e).prepend(t) : "append" == a ? $("#" + e).append(t) : $("#" + e).html(t), "post-body" == e && setTimeout(function() {
       ...
       BuzzyEditor.EditorInit(), App.initAjax()
      }, 500), void NProgress.done(!0))
     }
    }), !1
   })
  },
...
function checkResult(id,click_result)
{
 if ($.cookie('post_' + $('#post_id').attr('data-id')) == undefined) {
  $.getJSON("http://jsonip.com/?callback=?", function (data) {
   $.cookie('ip', data.ip, {expires: 360});
  });

  $.cookie('id', Math.floor((Math.random() * 100000) + 10000), {expires: 360});

  $.cookie('post_' + $('#post_id').attr('data-id'), id, {expires: 360});
 } else {
  return false;
 }

  if(click_result) {
    $('#href_'+id).addClass('green_bg');
  } else {
    $('#href_'+id).addClass('red_bg');
    setTimeout(function(){ 
      $('#href_'+id).removeClass('red_bg');
      $('#href_'+id).addClass('red_bgs');
        setTimeout(function(){ 
          $('#href_'+id).removeClass('red_bgs');
          $('#href_'+id).addClass('red_bg');
         
           setTimeout(function(){ 
            $('#href_'+id).removeClass('red_bg');
            $('#href_'+id).addClass('red_bgs');
           $('a[data-result=1]').addClass('green_bg');
            }, 350);
        }, 350);
    }, 350);
	...
	// var valueString=$("#answer_").val();
	// var dataArr = valueString.split("||");
 //  console.log(dataArr);
 ...
 //  		var idNew=dataArr2[0];
 //  		var checkNew=dataArr2[1];
 //  		if(checkNew==1){ 
 //        alert('check true');
 //  			$('#href_'+idNew).addClass('green_bg');
 //      } else if(checkNew != 1) {
 //        alert('check false');
 //        $('#href_'+id).addClass('red_bg');
...
$(document).on('keyup', 'input[name="textfield"]', function() {
 
 var token = $('input[name="_token"]').serialize();
 var form = $('#textfield').serialize() + '&' + token;
 $.ajax({
  type: "POST",
  url: '/gallerySearch',
  dataType: 'html',
  ...
  success: function (data) {
   if(data != 'false') {
    $('.srch_pic ul').html(data);
   } else {
    $('.srch_pic ul').html('<p>No results.</p>');
   }
  }
 });
});
...
//  if($('.mode-default').hasClass('mode-boxed')){
//   $('.header').show();
//   $('.post-header').show();
//   $('.post-headers').hide();
//  } else {
...
//   $('.mode-default').css('padding-top','0');
//   $('.content-wrapper').css('margin-top','0');
//   $('.container').css('padding','0');
//   $('.mainside').css('padding','0');
//
//  }
// });
...
$(document).scroll(function(){
 if($(this).scrollTop() > 1){
 $('.create_fixed').css('position','fixed');
  $('.create_fixed').css('top','50px');
  $('.create_fixed').css('bottom','auto');
 }else{
  $('.create_fixed').css('position','absolute');
  $('.create_fixed').css('top','auto');
  $('.create_fixed').css('bottom','0');
 }
});

// $('.drop-down').hover(function () {
//  $('.report_post').slideToggle();
// });


// var thismegapro = $('.answer').find('a.active');
// console.log(thismegapro);
// if($('.answer').find('a').hasClass('active')){
//   $('.answer').find('a').addClass('off');
//   $('.answer').find('a.active').removeClass('off')

// }

// $('.answer a').click(function(){
//   $('.answer').find('a').css('pointer-events','none');
//   $('.answer').find('a').addClass('off');
//   $('.answer').find('a.active').removeClass('off');
// })
// $('document').ready(function(){
//   $('.option-selection .result_bar_inner').css('background-color','#ffffff!important');
//   if($('.answer a').hasClass('active')){
//     $('.answer').find('a').css('pointer-events','none');
//   }
// });
$(document).ready(function(){
	if($('a.active').attr('data-result')==1){
        $('a.active').addClass('green_bg');
	} else {
		$('a.active').addClass('red_bgs');
	}
